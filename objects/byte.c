...
void bytearraySet(struct ByteArray *dest, const char* src)
{
  int len = strlen(src);
...
void bytearrayAdd(struct ByteArray *dest, const char* src)
{
  int len = strlen(src);
  ...
  if(nextlen > dest->size-1){
    if(dest->resizeable == true){
      bytearrayResize(dest, nextlen+1);
...
struct ByteArray bytearraySub(struct ByteArray *src,
  int start, int end)
// returns a substring from "start" to, but not including "end"
{
  ...
  struct ByteArray sub = newByteArray(end - start + 1);
  // get the substring
